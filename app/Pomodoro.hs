...
viewModel m = div_ [class_ "flex flex-col container items-center mx-auto justify-center pb-6 min-h-screen bg-neutral-200"] $ case m._pomodoroQueue of
  [] ->
    [ div_ [class_ "w-full absolute top-8"] $ case m._pomodoroPastQueue of
        [] -> []
        _ ->
          [ h1_ [class_ "text-center text-lg text-neutral-600 font-bold px-6"] [text "Pomodoro Ended!"],
            p_ [class_ "text-center prose text-neutral-600 px-4"] [text "Ready to start a new Pomodoro?"]
          ],
      settingsView True
    ]
