...
viewModel m =
  div_ [class_ "flex flex-col items-center justify-center gap-2 sm:gap-4 p-6 h-full"] $
    [ p_ [class_ "text-neutral-200 text-7xl sm:text-8xl font-mono"] [text . ms . formatTime defaultTimeLocale "%M:%00ES" $ m ^. timeLeft],
      div_ [class_ "grid grid-cols-2 place-items-center w-full items-center h-24 overflow-hidden"] $
        [ let (primaryAction, primarybuttonText) =
          ...
          button_
            [onClick End, class_ "self-center w-fit mx-auto text-neutral-200 text-4xl rounded px-4 py-2 hover:animate-wiggle hover:[animation-delay:0.25s]"]
            [ svg_
                [class_ "fill-neutral-200 size-16 hover:size-20 sm:size-20", xmlns_ "http://www.w3.org/2000/svg", viewBox_ "0 0 24 24"]
