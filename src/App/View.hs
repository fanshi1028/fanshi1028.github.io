...
instance Router RouteForTheOtherLang where
  routeParser =
    routes
      [ path "wasm" *> (RouteForTheOtherLang . to <$> gRouteParser),
        RouteForTheOtherLang Index <$ path "wasm"
      ]
  fromRoute = \case
    RouteForTheOtherLang Index -> [ toPath "wasm" ]
    RouteForTheOtherLang route' -> toPath "wasm" : gFromRoute (from route')
#endif
...
instance Router Route where
  routeParser =
    routes
      [ path "wasm" *> (to <$> gRouteParser),
        Index <$ path  "wasm"
      ]
  fromRoute = \case
    Index -> [ toPath "wasm" ]
    route' -> toPath "wasm" : gFromRoute (from route')
