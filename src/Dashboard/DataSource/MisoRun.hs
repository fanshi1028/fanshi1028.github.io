...
data MisoRunAction action a where
  MisoRunAction :: (Show action, Eq action) => action -> MisoRunAction action ()

deriving instance Eq (MisoRunAction action a)

deriving instance Show (MisoRunAction action a)

instance ShowP (MisoRunAction action) where showp = show

-- HACK: we don't care about the Hashable instance here, because we won't cache the result with `misoRunAction`
instance Hashable (MisoRunAction action a) where
  hashWithSalt s _ = hashWithSalt s (1 :: Int)

instance DataSourceName (MisoRunAction action) where
